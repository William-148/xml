from django.urls import path
from . import views

urlpatterns = [
    path('', views.index, name='index'),
    path('configuracion/', views.configuracion, name='configuracion'),
    path('consumo/', views.consumo, name='consumo'),
    path('inicializar_sistema/', views.inicializar_sistema, name='inicializar_sistema'),
    path('operaciones/', views.operaciones, name='operaciones'),
    path('recursos/', views.recursos, name='recursos'),
    path('recursos/crear', views.crearRecurso, name='crearRecurso'),
    path('categorias/', views.categorias, name='categorias'),
    path('categorias/crear', views.crearCategoria, name='crearCategoria'),
    path('clientes/', views.clientes, name='clientes'),
    path('clientes/crear', views.crearCliente, name='crearCliente'),
    path('clientes/instancia/<str:nitCliente>', views.formulario_instancia, name='formulario_instancia'),
    path('clientes/instancia/crear/<str:nitCliente>', views.crearClienteInstancia, name='crearClienteInstancia'),
    path('facturas/', views.facturas, name='facturas'),
    path('facturacion/', views.facturacion, name='facturacion'),
    path('reportes/', views.reportes, name='reportes'),
]

